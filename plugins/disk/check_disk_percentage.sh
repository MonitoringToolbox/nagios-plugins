...
# Description                                                                      #
# -------------------------------------------------------------------------------- #
# The purpose of this script is to determine the percentage of utilised storage    #
# for a specified mount point or path.                                             #
#                                                                                  #
#  Usage: check_disk_percentage.sh [ -h ] [ -m value ] [ -c value ] [ -w value ]   #
#    -h      : Print this screen                                                   #
#    -m      : Mount point                                                         #
...
function main()
{
    ...
    USED_DISK_SPACE=$(df -h "${MOUNT_POINT}" 2>/dev/null | grep -v Filesystem | awk '{print $5}' | sed 's/%//g')
    ...
    if (( $(echo "${USED_DISK_SPACE} >= ${CRITICAL_LEVEL}" | bc -l) )); then
        handle_critical "${USED_DISK_SPACE}% of disk space used on ${MOUNT_POINT}"
    elif (( $(echo "${USED_DISK_SPACE} >= ${WARNING_LEVEL}" | bc -l) )); then
        handle_warning "${USED_DISK_SPACE}% of disk space used on ${MOUNT_POINT}"
    elif (( $(echo "${USED_DISK_SPACE} >= 0" | bc -l) )); then
        handle_ok "${USED_DISK_SPACE}% of disk space used on ${MOUNT_POINT}"
    else
        handle_unknown "${USED_DISK_SPACE}% of disk space used on ${MOUNT_POINT}"
    fi
}
